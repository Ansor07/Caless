...
public class CalessEngine
{
    ...
    private void AddRabbitMoves(List<Move> moves, int row, int col, bool isWhite, int colorSign)
    {
        ...
        if (InBounds(nr, col))
        {
            ...
            if (target == EMPTY)
            {
                moves.Add(new Move(row, col, nr, col, board[row, col], EMPTY));
            ...
            // Атака вертикально вперёд
            if (target != EMPTY && target * colorSign < 0)
                moves.Add(new Move(row, col, nr, col, board[row, col], target));
        }
    }
    ...
    private void AddDragonMoves(List<Move> moves, int row, int col, bool isWhite, int colorSign, int bonus)
    {
        ...
        // Дальняя атака — захват ближайшего врага по вертикали и горизонтали
        ...
        for (int d = 0; d < orthoDirs.GetLength(0); d++)
        {
            ...
            for (int step = 2; step < BOARD_SIZE; step++)
            {
                ...
                if (target * colorSign < 0)
                {
                    Move rangedMove = new Move(row, col, nr, nc, board[row, col], target);
                    rangedMove.isDragonRanged = true;
                    bool alreadyAdded = false;
                    foreach (Move existing in moves)
                    {
                        if (existing.to.x == nr && existing.to.y == nc) { alreadyAdded = true; break; }
                    }
                    if (!alreadyAdded)
                        moves.Add(rangedMove);
                }
                break;
            }
        }
    ...
    private void AddRoosterMoves(List<Move> moves, int row, int col, bool isWhite, int colorSign)
    {
        ...
        // Ход вперёд (1 клетка)
        int nr = row + forward;
        if (InBounds(nr, col) && board[nr, col] == EMPTY)
        {
            moves.Add(new Move(row, col, nr, col, board[row, col], EMPTY));
    ...
    public List<Move> GetCastlingMoves(bool forWhite)
    {
        ...
        if (forWhite && (whiteCastleUsed || whiteDragonMoved || !whiteDragonAlive)) return castleMoves;
        if (!forWhite && (blackCastleUsed || blackDragonMoved || !blackDragonAlive)) return castleMoves;
 
        Vector2Int dragonPos = forWhite ? whiteDragonPos : blackDragonPos;
        int dcol = dragonPos.y;
 
        // Ищем фигуры на том же столбце
        int[] castleTypes = { PIG, GOAT, HORSE, DOG, RABBIT, TIGER };
 
        for (int r = 0; r < BOARD_SIZE; r++)
        {
            if (r == dragonPos.x) continue;
            int piece = board[r, dcol];
            ...
            // Проверяем путь — нет фигур между Драконом и целью
            int minR = Mathf.Min(dragonPos.x, r);
            int maxR = Mathf.Max(dragonPos.x, r);
            bool pathClear = true;
            for (int checkR = minR + 1; checkR < maxR; checkR++)
            {
                if (board[checkR, dcol] != EMPTY) { pathClear = false; break; }
            }
 
            if (pathClear)
            {
                // Дракон и фигура меняются местами
                Move castle = new Move(dragonPos.x, dcol, r, dcol, board[dragonPos.x, dcol], EMPTY);
                castle.isCastling = true;
                castle.castlePieceFrom = new Vector2Int(r, dcol);
                castle.castlePieceTo = dragonPos;
                castleMoves.Add(castle);
            }
        }
    ...
    public List<Move> GetTempleTeleportMoves(bool forWhite)
    {
        ...
        // Ищем все свои фигуры
        for (int r = 0; r < BOARD_SIZE; r++)
        {
            for (int c = 0; c < BOARD_SIZE; c++)
            {
                int piece = board[r, c];
                if (piece * colorSign <= 0) continue;
 
                // Телепортируем на любую пустую клетку
                for (int tr = 0; tr < BOARD_SIZE; tr++)
                {
                    for (int tc = 0; tc < BOARD_SIZE; tc++)
                    {
                        if (board[tr, tc] == EMPTY && (tr != r || tc != c))
                        {
                            Move teleport = new Move(r, c, tr, tc, piece, EMPTY);
