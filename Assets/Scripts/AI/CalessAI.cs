...
public class CalessAI
{
    ...
    public Move GetBestMove(CalessEngine eng)
    {
        ...
        // AI plays the side whose turn it is NOT the player's
        // Actually: engine.whiteTurn is the CURRENT turn. AI plays for current turn.
        forWhite = engine.whiteTurn;
        // But AI is called when it's AI's turn, so we use engine.whiteTurn directly
        // which should be the AI's color at this point
 
        List<Move> legalMoves = engine.GetAllLegalMoves(forWhite);
 
        // Добавляем рокировку
        List<Move> castlingMoves = engine.GetCastlingMoves(forWhite);
        foreach (Move cm in castlingMoves)
        {
            engine.MakeMove(cm, true);
            bool invalid = engine.IsRoyalUnderThreat(forWhite);
            engine.UndoLastMove();
            if (!invalid) legalMoves.Add(cm);
        }
